...
- **Responsivo/compatível com mobile** (decisão de 2026-07-24): o dashboard web também é a
  alternativa para quem **não quer usar o aplicativo** — toda tela precisa funcionar bem em
  celular (layout fluido, navegação utilizável em tela pequena), não só em desktop.
...
- **Lint (Biome)** — decisão de 2026-07-24: o monorepo usa Biome na raiz (config espelhada do
  eco-system-sales) como formatador + linter de backend/packages/mobile; o dashboard (Svelte) usa o
  Prettier/ESLint do próprio scaffold. **Após qualquer modificação de código, rodar `bun run lint`
  (e `bun run lint:fix` para aplicar correções) antes de dar a mudança por concluída** — mesma regra
  do typecheck. O pre-commit (Husky) roda o lint automaticamente, mas não substitui a validação
...
1. **M1 (atual)**: backend (auth com recuperação de senha + workspaces com fundação de multi-tenancy: workspace pessoal auto-criado no cadastro — junto com um banco e uma conta padrão, pra já dar pra lançar transação sem cadastro manual antes — `workspace_id` em todo o schema + CRUD de bancos, contas, cartões, categorias e transações + geração de faturas + soft delete + AuditLog write-only) e app (telas de bancos/contas/cartões/categorias/transações com criar/editar/excluir, busca/filtro por categoria na lista de transações, recorrências com confirmação de ocorrência pendente + onboarding com seed + widget de resumo financeiro). Previsão determinística básica (recorrências + parcelas futuras). *Sem UI de compartilhamento ainda — mas o schema já nasce pronto para isso.*
