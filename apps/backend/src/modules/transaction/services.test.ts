...
import { competencePeriod, splitInstallments } from "../../lib/invoices";
import {
  accountBalance,
  createTransaction,
  deleteTransaction,
...
import { listInvoices, payInvoice } from "../card/services";
...
describe("transações e saldo derivado", () => {
  test("receita/despesa afetam o saldo; transferência é neutra entre contas", async () => {
    ...
    expect(await accountBalance(db, accountId)).toBe(118_000);
    // reserva recebeu 200,00
    expect(await accountBalance(db, secondAccountId)).toBe(20_000);
  });
...
describe("crédito, fatura e pagamento", () => {
  ...
  test("pagar fatura cria transação de despesa na conta e marca paid; transação da fatura fica imutável", async () => {
    ...
    const balanceBefore = await accountBalance(db, accountId);
    ...
    expect(await accountBalance(db, accountId)).toBe(balanceBefore - 33_334);
