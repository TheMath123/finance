...
import { bankAccounts, cardInvoices, cards, categories, transactions } from "@finance/db";
...
import { listPendingOccurrences } from "../../recurring/services/list-pending-occurrences";
...
export async function monthlySummary(
  ...
  if (to >= today) {
    ...
    const unpaid = await db
      ...
      .where(
        and(eq(cardInvoices.workspaceId, actor.workspaceId), ne(cardInvoices.status, "paid")),
      );
    let unpaidDue = 0;
