...
import { accountBalance } from "../transaction/services";
import { monthlySummary } from "../summary/services";
...
describe("recorrências: previstas e confirmação", () => {
  test("fluxo: criar → aparece como prevista → confirmar → some da lista e vira transação", async () => {
    ...
    expect(await accountBalance(db, accountId)).toBe(balanceBefore + 300_000);
