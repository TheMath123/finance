...
export function occurrencesInMonth(
  ...
  if (rule.frequency === 'monthly') {
    return [
      iso(year, month, Math.min(rule.dayOfReference, daysInMonth(year, month))),
    ];
  }
...
export function validateRule(rule: {
  ...
  if (rule.frequency === 'weekly') {
    return (
      rule.dayOfReference >= 0 &&
      rule.dayOfReference <= 6 &&
      rule.monthOfReference == null
    );
  }
  ...
  if (rule.frequency === 'yearly') {
    return (
      rule.monthOfReference != null &&
      rule.monthOfReference >= 1 &&
      rule.monthOfReference <= 12
    );
  }
...
export function normalizeDescription(description: string): string {
  return description.toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '').trim();
}
