...
import { buildTransactionsCsv, type TransactionExportRow } from "./transactions-csv";
...
describe("buildTransactionsCsv", () => {
  ...
  test("escapa descrição com vírgula e aspas (RFC 4180)", () => {
    const csv = buildTransactionsCsv([row({ description: 'compra, com "aspas"' })]);
    expect(csv).toContain('"compra, com ""aspas"""');
  });

  test("formata parcela como número/total", () => {
    const csv = buildTransactionsCsv([row({ installmentNumber: 2, installmentTotal: 5, cardName: "Nubank", accountName: null })]);
