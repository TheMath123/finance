...
async function alreadyProcessed(deps: AppDeps, wamid: string): Promise<boolean> {
...
function extractTextMessages(payload: MetaWebhookPayload): { wamid: string; from: string; text: string }[] {
    ...
    for (const change of entry.changes ?? []) {
      ...
      for (const message of change.value?.messages ?? []) {
        if (message.type === "text" && message.text?.body) {
          messages.push({ wamid: message.id, from: normalizePhone(message.from), text: message.text.body });
        }
      }
    }
  }
...
function extractImageMessages(
  ...
  const messages: { wamid: string; from: string; mediaId: string; mimeType: string }[] = [];
  for (const entry of payload.entry ?? []) {
    for (const change of entry.changes ?? []) {
      if (change.field !== "messages") continue;
      for (const message of change.value?.messages ?? []) {
    ...
    .post("/whatsapp/webhook", async ({ request, set }) => {
      ...
      for (const { wamid, from, text } of messages) {
        if (await alreadyProcessed(deps, wamid)) {
          deps.httpLogger.warn({ scope: "whatsapp", wamid }, "webhook_message_replayed");
          continue;
        }
        void deps
      ...
      for (const { wamid, from, mediaId, mimeType } of images) {
        if (await alreadyProcessed(deps, wamid)) {
          deps.httpLogger.warn({ scope: "whatsapp", wamid }, "webhook_image_replayed");
          continue;
        }
        void deps
