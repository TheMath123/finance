...
export function createTrustedContactRepository(
  db: DbHandle
  ...
  return {
    findByPair: (userId, trustedUserId) =>
      db.query.trustedContacts.findFirst({
        where: and(
          ...
          eq(trustedContacts.trustedUserId, trustedUserId)
        ),
      }),
    listByUser: (userId) =>
      db.query.trustedContacts.findMany({
    ...
    async delete(id, userId) {
      await db
        ...
        .where(
          and(eq(trustedContacts.id, id), eq(trustedContacts.userId, userId))
        );
    },
  };
}
