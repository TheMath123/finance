...
export function createExpenseSplitRepository(
  db: DbHandle
  ...
  return {
    ...
    async activeTransactionIds(transactionIds) {
      ...
      const rows = await db
        ...
        .where(
          and(
            inArray(expenseSplits.transactionId, transactionIds),
            isNull(expenseSplits.cancelledAt)
          )
        );
