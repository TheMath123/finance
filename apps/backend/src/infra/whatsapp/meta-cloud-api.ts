...
async function sendOnce(to: string, body: string): Promise<void> {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `falha ao enviar mensagem no WhatsApp (status ${response.status}): ${errorBody}`
    );
  }
}
...
export async function fetchMediaUrl(
  ...
  if (!response.ok) {
    throw new Error(
      `falha ao resolver media_id do WhatsApp (status ${response.status})`
    );
  }
...
export async function downloadMedia(url: string): Promise<Uint8Array> {
  if (!isAllowedMediaHost(url)) {
    throw new Error(
      'URL de mídia do WhatsApp fora dos hosts esperados — download recusado'
    );
  }
  const { WHATSAPP_ACCESS_TOKEN } = env();
  ...
  if (!response.ok) {
    throw new Error(
      `falha ao baixar mídia do WhatsApp (status ${response.status})`
    );
  }
...
export function verifyWebhookSignature(
  ...
  const expectedHex = new Bun.CryptoHasher('sha256', env().WHATSAPP_APP_SECRET)
    .update(rawBody)
    .digest('hex');
