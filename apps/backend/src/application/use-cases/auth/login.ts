...
export async function login(
  ...
  if (!user) {
    ...
    await deps.hasher.verify(input.password, await deps.hasher.hash("dummy-password-for-timing"));
  ...
  if (!valid) {
    const { attempts, lockedUntil, lockMinutes } = nextLockoutState(user.failedLoginAttempts, now);
    await deps.repos.user.recordLoginFailure(user.id, attempts, lockedUntil);
    if (lockMinutes !== null) {
      deps.logger.log("account_locked", { userId: user.id, attempts, minutes: lockMinutes });
      await deps.dispatch("email.account-locked", { to: user.email, name: user.name, minutes: lockMinutes });
    } else {
