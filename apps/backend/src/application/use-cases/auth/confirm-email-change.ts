...
export async function confirmEmailChange(
  ...
  if (await deps.rateLimiter.isLimited(`email-change-confirm:${input.userId}`, 5, 15 * 60_000)) {
  ...
  if (conflicting && conflicting.id !== user.id) return left("email_already_in_use");
  ...
  await deps.dispatch("email.email-changed", { to: oldEmail, name: user.name, newEmail });
