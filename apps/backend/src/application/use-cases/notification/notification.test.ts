...
import { createInvite, createWorkspace } from '../workspace';
import {
  archiveNotification,
  createNotification,
...
describe('notification: criação respeita preferência', () => {
  test('cria a notificação e dispara push quando há token registrado', async () => {
    ...
    await registerPushToken(
      deps,
      actor.userId,
      `ExponentPushToken[${crypto.randomUUID()}]`
    );
...
describe('notification: ciclo de vida (ler, arquivar, desarquivar)', () => {
  test('marcar como lida e arquivar/desarquivar funcionam; dono é obrigatório', async () => {
    ...
    const strangerRead = await markNotificationRead(
      deps,
      stranger.userId,
      notification.id
    );
    ...
    if (!strangerRead.ok)
      expect(strangerRead.error).toBe('notification_not_found');
    ...
    const unarchived = await unarchiveNotification(
      deps,
      actor.userId,
      notification.id
    );
...
describe('notification: convite de workspace notifica o convidado', () => {
  test('convidar um usuário existente cria notificação workspace_invite pra ele', async () => {
    ...
    const family = await createWorkspace(deps, ownerActor.userId, {
      name: 'Família Notificação',
    });
    if (!family.ok) throw new Error('workspace falhou');
    const familyOwner: Actor = {
      userId: ownerActor.userId,
      workspaceId: family.value.id,
      role: 'owner',
    };
...
describe('notification: sweep diário (fatura fechou/vence, recorrência pendente)', () => {
  test('fatura open com closing_day no passado vira closed e notifica os membros', async () => {
    ...
    const updated = await db.query.cardInvoices.findFirst({
    ...
    expect(
      notifications.some(
        (n) =>
          n.type === 'invoice_closed' &&
          (n.data as { invoiceId?: string })?.invoiceId === invoice!.id
      )
    ).toBe(true);

    // Rodar de novo não duplica (dedup por entityKey).
    await runNotificationSweep(deps);
    const notificationsAfter = await listNotifications(
      deps,
      actor.userId,
      false
    );
  ...
  test('recorrência ativa com ocorrência hoje e não confirmada é lançada automaticamente e notifica (M2-09)', async () => {
    ...
    expect(
      notifications.some(
        (n) =>
          n.type === 'recurring_pending' &&
          (n.data as { recurringId?: string })?.recurringId === recurring!.id
      )
    ).toBe(true);

    // Rodar de novo não duplica o lançamento nem a notificação (dedup por recurringId+data / entityKey).
    await runNotificationSweep(deps);
    const allCreated = await db.query.transactions.findMany({
      where: eq(transactions.recurringId, recurring!.id),
    });
