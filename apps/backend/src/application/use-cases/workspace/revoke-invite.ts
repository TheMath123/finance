...
export async function revokeInvite(
  ...
  const role = invite
    ? await deps.repos.workspace.getMemberRole(invite.workspaceId, userId)
    : null;
  ...
  if (!invite || !role || !roleAtLeast(role, 'admin'))
    return left('invite_not_found');
  ...
  const revoked = await deps.repos.invite.updateStatus(
    invite.id,
    'pending',
    'revoked'
  );
