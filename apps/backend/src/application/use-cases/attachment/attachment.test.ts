...
async function newActorWithTransaction(
...
describe('uploadAttachment', () => {
  ...
  test('upload válido grava a key e permite leitura via URL assinada', async () => {
    ...
    if (urlResult.ok)
      expect(urlResult.value.url).toContain(uploaded.value.attachmentKey);
  });
  ...
  test('novo upload substitui o anexo anterior (não acumula lixo no storage)', async () => {
    ...
    await expect(
      deps.storage.getSignedReadUrl(first.value.attachmentKey, 60)
    ).rejects.toThrow();
  });
});
...
describe('getAttachmentUrl / deleteAttachment', () => {
  ...
  test('delete remove do storage e limpa a transação', async () => {
    ...
    if (!secondDelete.ok)
      expect(secondDelete.error).toBe('attachment_not_found');
  });
});
