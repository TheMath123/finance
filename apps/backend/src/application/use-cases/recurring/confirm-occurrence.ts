import { type Either, left } from '@finance/shared';
...
export async function confirmOccurrence(
  ...
  const rule = await deps.repos.recurring.findInWorkspace(
    actor.workspaceId,
    recurringId
  );
  ...
  if (!occurrencesInMonth(rule, y, m).includes(date))
    return left('not_an_occurrence');

  const existing = await deps.repos.transaction.findByRecurringAndDate(
    recurringId,
    date
  );
  ...
  try {
    ...
    if (isUniqueConstraintError(error))
      return left('occurrence_already_confirmed');
    throw error;
  }
}
