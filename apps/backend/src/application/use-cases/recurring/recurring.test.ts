...
describe('ocorrências (unidade)', () => {
  test('monthly clampa dia 31 para o fim do mês curto', () => {
    expect(
      occurrencesInMonth(
        { frequency: 'monthly', dayOfReference: 31, monthOfReference: null },
        2026,
        2
      )
    ).toEqual(['2026-02-28']);
...
describe('recorrências: previstas e confirmação', () => {
  test('fluxo: criar → aparece como prevista → confirmar → some da lista e vira transação', async () => {
    ...
    const confirmed = await confirmOccurrence(
      deps,
      actor,
      created.value.id,
      salary.date
    ...
    expect(
      after.find((p) => p.recurringId === created.value.id)
    ).toBeUndefined();

    // confirmar de novo falha
    const again = await confirmOccurrence(
      deps,
      actor,
      created.value.id,
      salary.date
  ...
  test('duas confirmações paralelas da mesma ocorrência: só uma vence, nunca duas transações', async () => {
    ...
    if (!occurrence)
      throw new Error('ocorrência não encontrada na lista de previstas');
    ...
    const materialized = await deps.repos.transaction.findByRecurringAndDate(
      created.value.id,
      occurrence.date
    );
...
describe('visão mensal e projeção', () => {
  test('summary traz receitas/despesas do mês e projeção com recorrências previstas', async () => {
    ...
    expect(summary.projectedAvailable!).toBeGreaterThanOrEqual(
      350_000 - 120_000
    );
  });
