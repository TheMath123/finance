...
export async function validateRefs(
  repos: Repositories,
  workspaceId: string,
  input: Pick<CreateRecurringInput, "method" | "categoryId" | "accountId" | "cardId">,
): Promise<RecurringError | null> {
  const category = await repos.category.findInWorkspace(workspaceId, input.categoryId);
  ...
  if (input.method === "credit") {
    ...
    const account = await repos.account.findInWorkspace(workspaceId, input.accountId);
