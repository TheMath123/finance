...
async function newUser(name: string): Promise<TestUser> {
  ...
  const accounts = await deps.repos.account.listByWorkspace(
    result.value.defaultWorkspaceId
  );
  const account = accounts[0];
  const category = await deps.repos.category.findFallback(
    result.value.defaultWorkspaceId
  );
  if (!account || !category)
    throw new Error('workspace de teste sem conta/categoria padrão');
  return {
...
describe('fluxo de confirmação (dois lados) + reembolso', () => {
  test('participante marca pago, criador confirma, gera reembolso e notifica', async () => {
    ...
    const early = await confirmShareReimbursement(
      deps,
    ...
    const wrongPayer = await markSharePaid(
      deps,
      { userId: creator.userId },
      share.id
    );
    ...
    const creatorNotifications = await listNotifications(
      deps,
      creator.userId,
      false
    );
    expect(
      creatorNotifications.some((n) => n.type === 'split_payment_paid')
    ).toBe(true);

    // Só o criador confirma.
    const wrongConfirmer = await confirmShareReimbursement(
      deps,
      { userId: p1.userId },
      share.id
    );
    ...
    const confirmed = await confirmShareReimbursement(
      deps,
      { userId: creator.userId },
      share.id
    );
    ...
    expect(notif.some((n) => n.type === 'split_reimbursement_confirmed')).toBe(
      true
    );
  });
...
describe('cancelSplit', () => {
  test('cancela quando nenhuma parte foi paga', async () => {
    ...
    const cancelled = await cancelSplit(
      deps,
      { userId: creator.userId },
      split.value.id
    );
...
describe('listTransactions: indicador hasActiveSplit (auditoria 2026-07-20)', () => {
  test('true com split ativo, false sem split e depois de cancelado', async () => {
    ...
    expect(afterCancel.find((t) => t.id === withSplit.id)?.hasActiveSplit).toBe(
      false
    );
  });
});
