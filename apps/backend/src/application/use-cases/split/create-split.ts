...
export async function createSplit(
  ...
  const existing =
    await deps.repos.expenseSplit.findActiveByTransaction(transactionId);
  ...
  if (hasManual) {
    if (
      resolved.some(
        (r) => !Number.isInteger(r.manualAmount) || r.manualAmount! <= 0
      )
    ) {
  ...
  const split = await deps.uow.run(async (repos) => {
    const createdSplit = await repos.expenseSplit.create({
      transactionId,
