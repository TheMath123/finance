...
export async function confirmShareReimbursement(
  ...
  const transaction = await deps.repos.transaction.findById(
    split.transactionId
  );
  if (!transaction?.accountId) throw new Error('transação do split inválida');
  ...
  const participantName =
    ...
    (share.participantUserId
      ? (await deps.repos.user.findById(share.participantUserId))?.name
      : undefined) ??
    'participante';
  ...
  try {
    updated = await deps.uow.run(async (repos) => {
      ...
      const result = await repos.splitShare.updateStatus(
        shareId,
        requiredStatus,
        'confirmed',
        reimbursement.id
    ...
    if (error instanceof AlreadyFinalizedError)
      return left('invalid_transition');
    throw error;
  }
