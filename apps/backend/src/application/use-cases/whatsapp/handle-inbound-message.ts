...
import { resolveAccountOrCard, routeChatbotMessage } from "../transaction/route-chatbot-message";
...
async function deterministicFallback(
  ...
  if (!result.ok) return `${prefix}Não consegui registrar — registra pelo app, por favor.`;
...
export async function handleInboundWhatsAppMessage(
  ...
  const obvious = await parseObviousTransaction(deps, workspaceId, message.text);
  if (obvious) {
    const result = await createTransaction(
      deps,
  ...
  try {
    const reply = await routeChatbotMessage(deps, workspaceId, user.id, message.text);
    await deps.tokenBudget.recordUsage(user.id, reply.usage.inputTokens + reply.usage.outputTokens);
    return { to: message.from, body: reply.body };
  } catch {
    const body = await deterministicFallback(deps, workspaceId, user.id, message.text, "");
    return { to: message.from, body };
  }
}
