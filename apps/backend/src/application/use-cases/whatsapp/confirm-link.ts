...
export async function confirmWhatsAppLink(
  deps: Pick<
    UseCaseDeps,
    'repos' | 'rateLimiter' | 'tokens' | 'uow' | 'dispatch' | 'notificationBus'
  >,
  phone: string,
  code: string
  ...
  const stored = await deps.repos.whatsappLinkCode.findValidByHash(
    deps.tokens.hashOpaque(code)
  );
  if (!stored) return left('invalid_code');

  const existingOwner = await deps.repos.user.findByPhone(phone);
  if (existingOwner && existingOwner.id !== stored.userId)
    return left('phone_already_linked');
  ...
  if (!user)
    throw new Error('usuário do código de vínculo do WhatsApp não encontrado');
