...
import { confirmWhatsAppLink, handleInboundWhatsAppMessage, revokeWhatsAppLink, startWhatsAppLink } from ".";
...
describe("whatsapp: confirmação do vínculo", () => {
  ...
  test("código já usado não pode ser reutilizado", async () => {
    ...
    const second = await confirmWhatsAppLink(deps, uniquePhone(), started.value.code);
  ...
  test("telefone já vinculado a outro usuário é rejeitado", async () => {
    ...
    const ownerConfirm = await confirmWhatsAppLink(deps, phone, ownerStart.value.code);
    ...
    const intruderConfirm = await confirmWhatsAppLink(deps, phone, intruderStart.value.code);
    expect(intruderConfirm.ok).toBe(false);
    if (!intruderConfirm.ok) expect(intruderConfirm.error).toBe("phone_already_linked");
  });
...
describe("whatsapp: roteamento de mensagem recebida (M2-06)", () => {
  test("número não vinculado + código de 6 dígitos confirma o vínculo", async () => {
    ...
    const reply = await handleInboundWhatsAppMessage(deps, { from: phone, text: started.value.code });
  ...
  test("número não vinculado + código errado não vincula e avisa", async () => {
    ...
    const reply = await handleInboundWhatsAppMessage(deps, { from: phone, text: "000000" });
  ...
  test("número não vinculado + texto qualquer só recebe instrução de vínculo", async () => {
    ...
    const reply = await handleInboundWhatsAppMessage(deps, { from: phone, text: "gastei 50 no mercado" });
