...
import { createDb, type Db, transactions } from '@finance/db';
...
async function newTransactionFor(
  deps: ReturnType<typeof createTestDeps>,
  actor: Actor
): Promise<string> {
...
describe('handleInboundWhatsAppImage', () => {
  ...
  test("segunda foto substitui a primeira (sem lógica especial de 'duas fotos')", async () => {
    ...
    await handleInboundWhatsAppImage(deps, {
      from: user.phone,
      buffer: new Uint8Array([1]),
      mimeType: 'image/jpeg',
    });
    const afterFirst = await deps.repos.transaction.findInWorkspace(
      user.actor.workspaceId,
      transactionId
    );
    ...
    const afterSecond = await deps.repos.transaction.findInWorkspace(
      user.actor.workspaceId,
      transactionId
    );
    ...
    await expect(
      deps.storage.getSignedReadUrl(firstKey!, 60)
    ).rejects.toThrow();
  });
});
