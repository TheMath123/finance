...
export async function startWhatsAppLink(
  ...
  if (await deps.rateLimiter.isLimited(`whatsapp-link-start:${userId}`, 3, 15 * 60_000)) {
  ...
  await deps.repos.whatsappLinkCode.create({ userId, codeHash: generated.hash, expiresAt });
