...
export async function createTransfer(
  deps: Pick<
    UseCaseDeps,
    'repos' | 'uow' | 'rateLimiter' | 'dispatch' | 'notificationBus'
  >,
  actor: Actor,
  ...
  if (
    await deps.rateLimiter.isLimited(
      `transfer-create:to:${recipient.id}`,
      30,
      60 * 60_000
    )
  ) {
  ...
  if (!sender || !category)
    throw new Error('workspace do ator sem usuário/categoria fallback');

  /** Destinatário já confia no remetente → pula o aceite manual (spec: contato confiável). */
  const trust = await deps.repos.trustedContact.findByPair(
    recipient.id,
    actor.userId
  );
  const trustedAccount = trust
    ? await deps.repos.account.findById(trust.defaultAccountId)
    : undefined;
  const autoAccept = Boolean(trustedAccount && !trustedAccount.archivedAt);

  const now = new Date();
  const expiresAt = new Date(
    now.getTime() + TRANSFER_EXPIRATION_DAYS * 24 * 60 * 60_000
  );

  const transfer = await deps.uow.run(async (repos) => {
    const fromDescription =
      input.description || `Transferência enviada para ${recipient.name}`;
    ...
    if (autoAccept && trustedAccount) {
      const toCategory = await repos.category.findFallback(
        trustedAccount.workspaceId
      );
