...
import { createDb, type Db, interUserTransfers } from '@finance/db';
...
describe('createTransfer', () => {
  ...
  test('cria transferência pendente, debita o remetente e notifica o destinatário', async () => {
    ...
    const fromTx = await deps.repos.transaction.findInWorkspace(
      sender.workspaceId,
      result.value.fromTransactionId
    );
    ...
    const pending = await listPendingTransfers(deps, {
...
describe('acceptTransfer / rejectTransfer', () => {
  test('destinatário aceita escolhendo conta — gera entrada e notifica o remetente', async () => {
    ...
    const toTx = await deps.repos.transaction.findInWorkspace(
      recipient.workspaceId,
      accepted.value.toTransactionId!
    );
    ...
    expect(notifications.some((n) => n.type === 'transfer_accepted')).toBe(
      true
    );
  });
  ...
  test('transferência já finalizada não pode ser aceita/recusada de novo', async () => {
    ...
    const secondAttempt = await rejectTransfer(
      deps,
      { userId: recipient.userId },
      created.value.id
    );
    ...
    if (!secondAttempt.ok)
      expect(secondAttempt.error).toBe('already_finalized');
  });
...
describe('contato confiável', () => {
  test('aceitar marcando confiável faz a próxima transferência entrar automático', async () => {
    ...
    const contacts = await listTrustedContacts(deps, {
      userId: recipient.userId,
    });
    ...
    const afterRemoval = await listTrustedContacts(deps, {
      userId: recipient.userId,
    });
...
describe('sweep de expiração', () => {
  test('transferência pendente vencida vira expired sem afetar a transação do remetente', async () => {
    ...
    const fromTx = await deps.repos.transaction.findInWorkspace(
      sender.workspaceId,
      created.value.fromTransactionId
    );
