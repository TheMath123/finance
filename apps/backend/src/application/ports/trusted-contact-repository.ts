...
export interface TrustedContactRepository {
  findByPair(userId: string, trustedUserId: string): Promise<TrustedContact | undefined>;
  ...
  upsert(userId: string, trustedUserId: string, defaultAccountId: string): Promise<TrustedContact>;
