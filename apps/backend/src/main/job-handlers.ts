...
} from '../application/use-cases/whatsapp';
...
export function createJobHandlers(
  ...
  return {
    ...
    'email.confirm-account-deletion': (p) =>
      mailer.sendConfirmAccountDeletion(p),
    'email.account-locked': (p) => mailer.sendAccountLocked(p),
    'email.workspace-invite': (p) => mailer.sendWorkspaceInvite(p),
    'push.send': (p) =>
      sendExpoPush(p.tokens, p.title, p.body, p.data, p.categoryId),
