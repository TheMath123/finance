<script lang="ts">
	...
	let kind = $state<'card' | 'account' | null>(null);
	...
	$effect(() => {
		if (open && !wasOpen) {
			step = showChoice ? 'choose' : cardCsvImportEnabled ? 'card-select' : 'account-select';
			kind = showChoice ? null : cardCsvImportEnabled ? 'card' : 'account';
			selectedCardId = activeCards[0]?.id ?? '';
	...
	<Dialog.Content class="sm:max-w-2xl">
		...
		{:else if step === 'done' && summary}
			<div class="grid gap-3">
				...
				{/if}
				<Dialog.Footer>
					<Button type="button" onclick={finish}>Fechar</Button>
				</Dialog.Footer>
			</div>
		{/if}
	</Dialog.Content>
