import { CaretRightIcon, TrendDownIcon, TrendUpIcon } from 'phosphor-react-native';
 ...
 */
export function BalanceOverview({ summary }: { summary: MonthlySummary | undefined }) {
    ...
    <View className="w-full gap-4">
      <View className="w-full flex-row items-center justify-center px-4 py-3.5">
        <View className="flex-1 gap-2.5">
          <Text className="text-[10px] font-semibold leading-tight text-foreground">Saldo disponível</Text>
          <Text className="text-[20px] font-normal leading-tight text-foreground">{formatCents(balance)}</Text>
        </View>
      ...
      <View className="w-full flex-row gap-4 px-4">
        <View className="flex-1 flex-row items-center gap-4 rounded-lg border border-foreground/10 p-4">
          ...
          <View className="gap-1">
            <Text className="text-[10px] font-semibold leading-tight text-foreground">Receitas</Text>
            <Text className="text-[16px] font-normal leading-tight text-foreground">{formatCents(summary?.income ?? 0)}</Text>
          </View>
        </View>
        <View className="flex-1 flex-row items-center gap-4 rounded-lg border border-foreground/10 p-4">
          <TrendDownIcon size={16} color={BrandColors.destructive} />
          <View className="gap-1">
            <Text className="text-[10px] font-semibold leading-tight text-foreground">Despesas</Text>
            <Text className="text-[16px] font-normal leading-tight text-foreground">{formatCents(summary?.expense ?? 0)}</Text>
          </View>
        </View>
      </View>
    </View>
