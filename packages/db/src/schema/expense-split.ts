...
export const splitShareStatusEnum = pgEnum("split_share_status", ["pending", "paid", "confirmed"]);
...
export const expenseSplitsRelations = relations(expenseSplits, ({ one, many }) => ({
  transaction: one(transactions, { fields: [expenseSplits.transactionId], references: [transactions.id] }),
  createdByUser: one(users, { fields: [expenseSplits.createdBy], references: [users.id] }),
...
export const splitShares = pgTable(
  "split_shares",
  {
    ...
    participantUserId: uuid("participant_user_id").references(() => users.id, { onDelete: "cascade" }),
    ...
    reimbursementTransactionId: uuid("reimbursement_transaction_id").references(() => transactions.id, {
      onDelete: "restrict",
    }),
    createdAt: createdAt(),
    updatedAt: updatedAt(),
  ...
  (t) => [
    ...
    index("split_shares_participant_user_idx").on(t.participantUserId, t.status),
  ],
);

export const splitSharesRelations = relations(splitShares, ({ one }) => ({
  split: one(expenseSplits, { fields: [splitShares.splitId], references: [expenseSplits.id] }),
  participantUser: one(users, { fields: [splitShares.participantUserId], references: [users.id] }),
