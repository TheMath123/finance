...
export const whatsappLinksRelations = relations(whatsappLinks, ({ one }) => ({
  workspace: one(workspaces, { fields: [whatsappLinks.workspaceId], references: [workspaces.id] }),
  linkedByUser: one(users, { fields: [whatsappLinks.linkedBy], references: [users.id] }),
}));
