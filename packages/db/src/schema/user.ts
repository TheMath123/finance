...
export const users = pgTable(
  'users',
  {
    ...
    updatedAt: updatedAt(),
    /**
     * Busca full-text por nome + e-mail (painel `/saas/users`) — gerada pelo
     * Postgres. Ver CLAUDE.md: buscas por palavra usam full-text search,
     * nunca ILIKE.
     */
    searchVector: tsvector('search_vector').generatedAlwaysAs(
      (): SQL =>
        sql`to_tsvector('portuguese', ${users.name} || ' ' || ${users.email})`
    ),
  },
  (t) => [index('users_search_vector_idx').using('gin', t.searchVector)]
);
